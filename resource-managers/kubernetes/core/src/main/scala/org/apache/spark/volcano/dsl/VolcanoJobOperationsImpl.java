...
public class VolcanoJobOperationsImpl extends HasMetadataOperation<Job, JobList, DoneableJob, Resource<Job, DoneableJob>> {
    ...
    public Job updateReplicas(Job job, int replicas) {
        ...
        return this.patch(job, Collections.singletonList(operation));
    }

    protected <Job> Job handlePatch(Job current, List<Map<String, Object>> parameters, Class<Job> type) throws ExecutionException, InterruptedException, IOException {
