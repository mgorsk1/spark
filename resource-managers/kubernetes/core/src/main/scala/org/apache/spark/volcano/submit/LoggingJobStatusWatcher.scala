...
class LoggingJobStatusWatcherImpl(conf: KubernetesDriverConf)
  ...
  def checkTaskStatus(phase: String): String = {
    // This is to handle the situation where the driver job succeeds but the
    // driver Task fails. Currently we have to check the status of the underlying
    // tasks
    if ("Completed".equals(phase) && job.map(_.getStatus).get.getFailed > 0) {
      "Failed"
    }
    else {
      phase
  ...
  private def phase: String = {
    ...
    if (status.isDefined && status.get != null) {
      ...
      if (Option(state).isDefined) checkTaskStatus(state.getPhase) else "Unknown"
    } else {
      "Unknown"
    }
  }
